...
/**
 * Represents the main objects in simulation, each may have a set of parts
 * (through the intermediate part application class), and a set of states, with
 * one current state.
 * 
 * @author Paul Grogan
 */
public class Element extends DomainObject implements I_Element {
	...
	/**
	 * The default constructor sets the default class of supply to COS0, sets
	 * the environment to unpressurized, and initializes the set of part
	 * applications and the set of states. Also creates a default state, adds it
	 * to the set of states, and sets it as the current state.
	 */
	public Element() {
		super();
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.domain.element.I_Element#setClassOfSupply(edu.mit.spacenet.domain.ClassOfSupply)
	 */
	...
	public Environment getEnvironment() {
		return GlobalParameters.isEnvironmentConstrained()?
				environment:Environment.UNPRESSURIZED;
	}
	
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.domain.element.I_Element#setEnvironment(edu.mit.spacenet.domain.Environment)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.domain.element.I_Element#generateDemands(double, edu.mit.spacenet.simulator.I_Simulator)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.domain.element.I_Element#satisfyDemands(edu.mit.spacenet.domain.resource.DemandSet, edu.mit.spacenet.simulator.I_Simulator)
	 */
	public void satisfyDemands(DemandSet demands, I_Simulator simulator) {
		if(simulator.getScenario().isScavengeSpares()
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.domain.element.I_Element#getTotalMass(edu.mit.spacenet.domain.ClassOfSupply)
	 */
	public double getTotalMass(ClassOfSupply cos) {
		double mass = 0;
		if(GlobalParameters.isScavengeSpares()
				&& getCurrentState() != null
				&& getCurrentState().getStateType().equals(StateType.DECOMMISSIONED)) {
			double partsMass = 0;
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.domain.element.I_Element#setCurrentState(edu.mit.spacenet.domain.element.I_State)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.domain.element.I_Element#setContainer(edu.mit.spacenet.domain.I_Container)
	 */
	...
	public Location getLocation() {
		...
		else if(getContainer() instanceof Location) return (Location)getContainer();
		else if(getContainer() instanceof I_Carrier) return ((I_Carrier)getContainer()).getLocation();
		else throw new RuntimeException("Illegal container: " + getContainer());
	}
	...
	public int compareTo(I_Element element) {
		if(getName().equals(element.getName())) {
			if(getTid() == element.getTid()) {
				return new Integer(getUid()).compareTo(
						new Integer(element.getUid()));
			} else {
				return new Integer(getTid()).compareTo(
						new Integer(element.getTid()));
			}
		} else {
	...
	public ImageIcon getIcon() {
		if(iconType==null) return getElementType().getIconType().getIcon();
		else if(icon==null) icon = iconType.getIcon();
		return icon;
	}
