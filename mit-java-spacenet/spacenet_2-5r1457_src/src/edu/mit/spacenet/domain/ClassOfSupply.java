...
/**
 * This enumeration lists the ten main classes of supply, and the sub-classes
 * of supply.
 * 
 * @author Paul Grogan
 */
public enum ClassOfSupply {
	...
	public Color getColor() {
		if(color==null) return getBaseClass().getColor();
		else return color;
	}
	...
	/**
	 * Determines whether this class of supply is an instance (equal to or a
	 * subclass of) the given class of supply.
	 * 
	...
	public boolean isInstanceOf(ClassOfSupply cos) {
		if(this.equals(cos) || this.isSubclassOf(cos)) return true;
		else return false;
	}
	
	/**
	 * Determines whether this class of supply is a subclass (i.e. child, more
	 * specific) of the given class of supply.
	 * 
	 * @param cos the class of supply to compare against
	 * 
	...
	public boolean isSubclassOf(ClassOfSupply cos) {
		...
		if(superclassId.length() >= subclassId.length()) return false;
		if(id == 10 || cos.getId() == 10) return false;
		// above is a HACK to account for poor naming convention
		for(int i = 0; i < superclassId.length(); i++) {
			if(superclassId.charAt(i) != subclassId.charAt(i)) return false;
		}
		return true;
	}
	
	/**
	 * Determines whether this class of supply is a superclass (i.e. parent,
	 * less specific) of the given class of supply.
	 * 
	 * @param cos the class of supply to compare against
	 * 
