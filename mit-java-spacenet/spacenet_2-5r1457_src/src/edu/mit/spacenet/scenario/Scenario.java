...
/**
 * The scenario class is used to store all information used in a simulation. It
 * is the class that is serialized to a file when saving.
 * 
 * @author Paul Grogan
 */
public class Scenario {
	...
	/**
	 * The default constructor sets a default name and date, chooses the lunar
	 * scenario type, and initializes the data structures.
	 */
	public Scenario() {
	...
	public List<Object> getNodeUses(Node node) {
		...
		for(Mission m : getMissionList()) {
			if(node.equals(m.getOrigin())
					|| node.equals(m.getDestination())
					|| node.equals(m.getReturnOrigin())
					|| node.equals(m.getReturnDestination()))
				uses.add(m);
			for(I_Event e : m.getEventList()) {
				if(e.getLocation().equals(node)
						|| (e instanceof I_Transport
						&& (node.equals(((I_Transport)e).getOrigin())
						|| node.equals(((I_Transport)e).getDestination())))) {
					if(!uses.contains(m)) uses.add(m);
					uses.add(e);
				}
			}
		}
		...
		for(Mission m : getMissionList()) {
			for(I_Event e : m.getEventList()) {
				if(e instanceof I_Transport
						&& edge.equals(((I_Transport)e).getEdge())) {
					uses.add(m);
					uses.add(e);
				}
			}
	...
	/**
	 * Gets whether detailed EVAs should be used in simulation. Detailed EVAs
	 * generate Move, Reconfigure, and Demand events and can bog down large
	 * simulations.
	 * 
	...
	/**
	 * Sets whether detailed EVAs should be used in simulation. Detailed EVAs
	 * generate Move, Reconfigure, and Demand events and can bog down large
	 * simulations.
	 * 
	...
	/**
	 * Sets whether detailed explorations should be used in simulation. Detailed
	 * explorations generate EVA events and can bog down large simulations.
	 * 
	...
	/**
	 * Gets whether detailed explorations should be used in simulation. Detailed
	 * explorations generate EVA events and can bog down large simulations.
	 * 
	...
	/**
	 * Sets the item aggregation point.
	 * param itemAggregation	the item aggregation (0=pre-aggregation, 1=post-aggregation)
	 * 
