...
/**
 * A mission is usually described as one crew rotation or the unmanned delivery
 * of supplies.
 * 
 * @author Paul Grogan
 */
public class Mission implements Comparable<Mission> {
	...
	/**
	 * Sets the name of the mission and updates and member elements' prefix
	 * names appropriately with the first five letters.
	 * 
	...
	public void setName(String name) {
		...
		for(I_Event event : getEventList()) {
			if(event.getName().contains(" | "))
				event.setName(getName().substring(0,Math.min(getName().length(), 5)) + event.getName().substring(event.getName().indexOf(" | ")));
			if(event instanceof CreateEvent) {
				for(I_Element element : ((CreateEvent)event).getElements()) {
					if(element.getName().contains(" | "))
						element.setName(getName().substring(0,Math.min(getName().length(), 5)) + element.getName().substring(element.getName().indexOf(" | ")));
				}
			}
		}
	}
	...
	private double getRemainingCapacityTo(Node node) {
		...
		for(I_Event e : getEventList()) {
			if(e instanceof FlightTransport 
				...
				preSim.simulate(e.getTime() + DateFunctions.getDaysBetween(getStartDate(), getScenario().getStartDate()), e.getPriority(), false);
				for(I_Element element : ((FlightTransport)e).getElements()) {
					if(element instanceof I_Carrier && preSim.getScenario().getNetwork().getRegistrar().containsKey(element.getUid())) {
						capacity -= ((I_Carrier)preSim.getScenario().getNetwork().getRegistrar().get(element.getUid())).getCargoMass();
					}
				}
				...
				for(I_Element element : ((SpaceTransport)e).getElements()) {
					if(element instanceof I_Carrier && preSim.getScenario().getNetwork().getRegistrar().containsKey(element.getUid())) {
						capacity += ((I_Carrier)element).getMaxCargoMass()-((I_Carrier)element).getCargoMass();
					}
				}
			}
		}
	...
	/**
	 * Gets the set of delivery carriers in the mission (elements that arrive at
	...
	public Set<I_Carrier> getDeliveryCarriers() {
		...
		for(I_Event e : getEventList()) {
			if(e instanceof I_Transport
					&& ((I_Transport)e).getDestination().equals(getDestination())) {
	...
	/**
	 * Gets the set of return carriers in the missino (elements that arrive at
	 * the mission return destination).
	 * 
	 * @return the set of carriers
	 */
	...
	public double getEvaCrewTime() {
		...
		for(I_Event e : getEventList()) {
			if(e instanceof EvaEvent) {
				...
				time += ((ExplorationProcess)e).getNumberEva()*((ExplorationProcess)e).getEvaDuration()*((ExplorationProcess)e).getStateMap().size();
			}
		}
		return time;
	}
	...
	public double getDuration() {
		...
		if(this.equals(getScenario().getMissionList().get(getScenario().getMissionList().size()-1))) {
			...
			for(I_Event e : getEventList()) {
				...
				if(e instanceof I_Process) newEndTime += ((I_Process)e).getDuration();
				if(newEndTime > endTime) endTime = newEndTime;
			}
		} else {
			// else the end time is the days between the start of the scenario
			// and the start of the next mission
			endTime = DateFunctions.getDaysBetween(getScenario().getMissionList().get(getScenario().getMissionList().indexOf(this)+1).getStartDate(), 
	...
	public double getDestinationDuration() {
		...
		for(I_Event e : getEventList()) {
			if(e instanceof I_Transport) {
				if(((I_Transport)e).getDestination().equals(getDestination())
						&& arrival == -1) {
		...
		else if(departure < 0) return GlobalParameters.getRoundedTime(getDuration() - arrival);
		else return GlobalParameters.getRoundedTime(departure - arrival);
	}
	
	/**
	 * Gets the duration in transit to the destination node. The transit
	 * duration is defined as the time elapsed from the start of the mission to
	 * the arrival of a transport event at the mission destination node.
	 * 
	...
	/**
	 * Gets the return duration in transit from the destination node. The return
	 * transit duration is defined as the time elapsed from the start of the
	 * last transport event to leave the mission return origin node to the
	 * arrival of the last transport event to arrive at the mission return
	 * destination node.
	 * 
	 * @return the transit duration (days)
	 */
	public double getReturnTransitDuration() {
		...
		return GlobalParameters.getRoundedTime((transitEnd!=-1&&transitStart!=-1) ? transitEnd - transitStart : 0);
	}
	...
	public double getDormantDuration() {
		return GlobalParameters.getRoundedTime(getDuration() - getTransitDuration() - getDestinationDuration() - getReturnTransitDuration());
	}
