...
/**
 * Visualization of the network with options to run animation over the course
 * of the simulation.
 * 
 * @author Paul Grogan
 */
public class NetworkHistoryVisualization extends JSplitPane {
	...
	private JButton startButton, rewindButton, stopButton,
		fastforwardButton, endButton;
	...
	private JLabel stateLabel, stateValueLabel, massLabel, massValueLabel, 
		volumeLabel, volumeValueLabel;
	...
	public NetworkHistoryVisualization(SimulationTab tab) {
		...
		startButton = new JButton(new ImageIcon(getClass().getClassLoader().getResource("icons/control_start.png")));
		...
		rewindButton = new JButton(new ImageIcon(getClass().getClassLoader().getResource("icons/control_rewind.png")));
		...
		stopButton = new JButton(new ImageIcon(getClass().getClassLoader().getResource("icons/control_stop.png")));
		...
		playButton = new JToggleButton(new ImageIcon(getClass().getClassLoader().getResource("icons/control_play.png")));
		playButton.setSelectedIcon(new ImageIcon(getClass().getClassLoader().getResource("icons/control_pause.png")));
		...
		fastforwardButton = new JButton(new ImageIcon(getClass().getClassLoader().getResource("icons/control_fastforward.png")));
		...
		endButton = new JButton(new ImageIcon(getClass().getClassLoader().getResource("icons/control_end.png")));
	...
	public void updateView() {
		...
		if(tab.getSimulator().getNetworkHistory().size() > 0) {
			...
			while(maxVal%numLabels!=0) maxVal++;
			timeSlider.setMaximum(maxVal);
			Hashtable<Integer, JLabel> labelDictionary = new Hashtable<Integer, JLabel>();
			SimpleDateFormat dateFormat = new SimpleDateFormat("MMM d, yyyy");
			for(int i=0; i<=numLabels; i++) {
				Date date = DateFunctions.getDate(getScenario().getStartDate(), maxVal*i/numLabels/stepsPerDay);
	...
	private void updateElements() {
		if(tab.getSimulator().getNetworkHistory().size()>0) {
			...
			if(timeSlider.getValue()==timeSlider.getMaximum()) {
				...
				int i=0;
				while(tab.getSimulator().getNetworkHistory().get(i+1).getTime()*stepsPerDay <= timeSlider.getValue()
			...
			dateLabel.setText("Date: " + dateFormat.format(DateFunctions.getDate(getScenario().getStartDate(), time))); 
	...
	private void updateCapacities() {
		...
		if(elementTree.getSelection() instanceof I_Carrier) {
			...
			capacityPanel.updateCapacities(carrier, carrier.getCargoMass(), 
					carrier.getCargoVolume(), carrier.getCrewSize());
			...
			capacityPanel.setVisible(true);
			capacityPanel.updateCapacities(location, location.getCargoMass(), 
					location.getCargoVolume(), location.getCrewSize());
		} else {
			capacityPanel.setVisible(false);
		}
	...
	class Animation extends SwingWorker<Void, Void> {
		...
		public Void doInBackground() {
			if(constantTimeCheck.isSelected()) {
				while(timeSlider.getValue() < timeSlider.getMaximum() 
						&& !isCancelled()) {
				...
				int i = 0;
				while(i < tab.getSimulator().getNetworkHistory().size() 
						&& timeSlider.getValue()/stepsPerDay > tab.getSimulator().getNetworkHistory().get(i).getTime()) {
					i++;
				}
				while(i < tab.getSimulator().getNetworkHistory().size()
						&& !isCancelled()) {
					timeSlider.setValue((int)(tab.getSimulator().getNetworkHistory().get(i).getTime()*stepsPerDay));
					i++;
					try {
						Thread.sleep((1000/(Integer)rateSpinner.getValue()));
					} catch (InterruptedException e) { }
