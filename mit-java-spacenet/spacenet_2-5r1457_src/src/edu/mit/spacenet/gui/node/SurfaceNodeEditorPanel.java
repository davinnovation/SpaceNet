...
public class SurfaceNodeEditorPanel extends AbstractNodeEditorPanel {
	...
	private void initialize() {
		if(node!= null){
			...
			if(node.getLatitude()<0) latitudeCombo.setSelectedItem(S);
			else latitudeCombo.setSelectedItem(N);
			longitudeModel.setValue(Math.abs(node.getLongitude()));
			if(node.getLatitude()<0) longitudeCombo.setSelectedItem(W);
			else longitudeCombo.setSelectedItem(E);
	...
	public boolean isNodeValid() {
		...
		if(bodyCombo.getSelectedItem()==null) {
			JOptionPane.showMessageDialog(getDialog(), 
					"Please select a celestial body.",
					"SpaceNet Warning",
					JOptionPane.WARNING_MESSAGE);
			return false;
		}
	...
	public void saveNode(){
		...
		node.setLatitude((latitudeCombo.getSelectedItem()==S?-1:1)*latitudeModel.getNumber().doubleValue());
		node.setLongitude((longitudeCombo.getSelectedItem()==W?-1:1)*longitudeModel.getNumber().doubleValue());
