...
/**
 * Component for visualizing the feasibility of a scenario based on the used and
...
public class ScenarioFeasibilityTab extends JSplitPane {
	...
	private class OptionsPanel extends JPanel {
		...
		private void buildPanel() {
			...
			dataTable.getColumnModel().getColumn(0).setHeaderRenderer(new VisibilityTableCellHeaderRenderer());
	...
	private class ChartPanel extends SNChartPanel {
		...
		public void updateChart() {
			JFreeChart chart;
			if(optionsPanel.cumulativeCheck.isSelected()
					&& optionsPanel.timeDomainCheck.isSelected()) {
		...
		private JFreeChart buildTimeSeriesChart() {
			...
			for(SupplyEdge edge : optionsPanel.supplyEdgesModel.getAllObjects()) {
				Hour hour = new Hour(DateFunctions.getDate(demandsTab.getScenario().getStartDate(), edge.getStartTime()));
				...
				estimatedDemands += demandsTab.getSimulator().getAggregatedEdgeDemands().get(edge).getTotalMass();
		...
		private JFreeChart buildBarChart() {
			...
			for(SupplyEdge edge : optionsPanel.supplyEdgesModel.getSelectedObjects()) {
				if(optionsPanel.dataModel.getSelectedObjects().contains(ESTIMATED_DEMANDS)
					...
					dataset.addValue(demandsTab.getSimulator().getAggregatedEdgeDemands().get(edge).getTotalMass() 
							+ demandsTab.getSimulator().getAggregatedNodeDemands().get(edge.getPoint()).getTotalMass(),
