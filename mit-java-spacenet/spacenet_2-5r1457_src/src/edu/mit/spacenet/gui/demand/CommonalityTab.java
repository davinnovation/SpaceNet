...
public class CommonalityTab extends JSplitPane {
	...
	/**
	 * Requests that the component and associated charts update based on the
	 * underlying model.
	 */
	...
	private class OptionsPanel extends JPanel {
		...
		private void buildPanel() {
			...
			partsTable.getColumnModel().getColumn(0).setHeaderRenderer(new VisibilityTableCellHeaderRenderer());
	...
	private class ChartPanel extends SNChartPanel {
		...
		public void updateChart() {
			...
			JFreeChart chart = ChartFactory.createTimeSeriesChart(
					null, 
					null, 
					optionsPanel.optionCombo.getSelectedItem().equals(CUMULATIVE_UNITS)?"Units":"Mass (kg)", 
					dataset, 
					!optionsPanel.optionCombo.getSelectedItem().equals(CUMULATIVE_MASS), // legend
					true, 
					false);
			...
			if(optionsPanel.optionCombo.getSelectedItem().equals(CUMULATIVE_UNITS)) {
				for(I_Item item : optionsPanel.partsModel.getSelectedObjects()) {
					...
					for(SimScavenge scavenge : demandsTab.getSimulator().getScavengedParts()) {
						if(optionsPanel.elementsModel.getSelectedObjects().contains(scavenge.getElement())
							...
							s.addOrUpdate(new Hour(DateFunctions.getDate(getScenario().getStartDate(), scavenge.getTime())).previous(), amount);
							amount += scavenge.getAmount();
							s.addOrUpdate(new Hour(DateFunctions.getDate(getScenario().getStartDate(), scavenge.getTime())), amount);
						}
					}
					dataset.addSeries(s);
				...
				for(I_Element element : optionsPanel.elementsModel.getSelectedObjects()) {
					TimeSeries s = new TimeSeries(element.getName() + " [" + element.getUid() + "]", Hour.class);
					double amount = 0;
					for(SimScavenge scavenge : demandsTab.getSimulator().getScavengedParts()) {
