...
/**
 * Component for showing the demands filtered by mission.
 * 
 * @author Paul Grogan
 */
public class MissionDemandsTab extends JSplitPane {
	...
	public MissionDemandsTab(DemandsTab demandsTab) {
		...
		private void buildPanel() {
			...
			missionsModel.addTableModelListener(new TableModelListener() {
				public void tableChanged(TableModelEvent e) {
					...
					displayTotalCheck.setEnabled(missionsModel.getSelectedObjects().size()==1);
					smoothDemandsCheck.setEnabled(missionsModel.getSelectedObjects().size()==1);
				}
			});
			...
			JScrollPane elementsScroll = new JScrollPane(missionsTable);
			...
			selectAllElementsButton.setToolTipText("Select All Missions");
			...
			deselectAllElementsButton.setToolTipText("Deselect All Missions");
	...
	private class ChartPanel extends SNChartPanel {
		...
		public void updateChart() {
			if(optionsPanel.missionsModel.getSelectedObjects().size()==1) {
		...
		private JFreeChart buildTimeSeriesChart() {
			...
			for(SimDemand demand : demandsTab.getSimulator().getDemandHistory(optionsPanel.missionsModel.getSelectedObjects().get(0))) {
		...
		private JFreeChart buildBarChart() {
			...
			for(Mission mission : optionsPanel.missionsModel.getSelectedObjects()) {
				for(int i=1; i<=10; i++) {
					if(optionsPanel.cosModel.getSelectedObjects().contains(ClassOfSupply.getInstance(i))) {
						dataset.addValue(demandsTab.getSimulator().getDemands(mission, ClassOfSupply.getInstance(i)).getTotalMass(),
								"COS " + i, mission.getName());
