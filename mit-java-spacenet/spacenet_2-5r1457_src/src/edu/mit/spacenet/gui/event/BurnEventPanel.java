...
public class BurnEventPanel extends AbstractEventPanel {
		...
		ddlBurnType.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				...
				if(!ddlBurnType.getSelectedItem().equals(event.getBurn().getBurnType())) {
					event.getBurn().setBurnType(BurnType.getInstance(ddlBurnType.getSelectedItem().toString()));
					updateView();
				}
			}
		});
	...
	public void saveEvent() {
		event.setBurn(new Burn(0, 
				BurnType.getInstance(ddlBurnType.getSelectedItem().toString()), 
	...
	public void updateView() {
		...
		for(Object o : sequenceModel.toArray()) {
			...
			if(!isFound) sequenceModel.clear();
		}
		updateDeltaV();
	}
	private void updateButtons() {
		if(elementTree.getSelectionCount() > 0) {
			if(elementTree.getSelectionCount() == 1 
					...
					&& elementModel.getTopCheckedElements().contains(((CheckBoxNode)elementTree.getSelectionPath().getLastPathComponent()).getElement())) {
				boolean isStaged = false;
				for(Object o : sequenceModel.toArray()) {
					if(((BurnStageItem)o).getElement().equals(((CheckBoxNode)elementTree.getSelectionPath().getLastPathComponent()).getElement())
				...
				if(isStaged) {
					...
					if(((CheckBoxNode)elementTree.getSelectionPath().getLastPathComponent()).getElement() instanceof PropulsiveVehicle) {
						...
						if((event.getBurn().getBurnType().equals(BurnType.OMS) && v.getOmsFuelTank() != null)
								||(event.getBurn().getBurnType().equals(BurnType.RCS) && v.getRcsFuelTank() != null))
	...
	private void updateDeltaV() {
		...
		deltaV.setValue((int)(100*((burnDeltaVModel.getNumber().doubleValue() - deltaVReq)/burnDeltaVModel.getNumber().doubleValue())));
		deltaV.setString(format.format(burnDeltaVModel.getNumber().doubleValue() - deltaVReq) + " / " + burnDeltaVModel.getNumber() + " m/s");	
	}
}
