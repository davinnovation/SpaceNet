...
/**
 * An event that models the surface transport along a surface edge using a 
 * surface vehicle.
 * 
 * @author Paul Grogan
 */
public class SurfaceTransport extends AbstractEvent implements I_Transport {
	...
	public void execute(I_Simulator simulator) throws SimError {
		if(edge == null) {
			throw new SimSpatialError(simulator.getTime(), this, 
					"No surface edge defined.");
		}
		if(vehicle == null) {
			throw new SimSpatialError(simulator.getTime(), this, 
				"No surface vehicle defined.");
		}
		...
		if(transportState != null) {
			...
			r1.setParent(this);
			if(isReversed()) r1.setLocation(edge.getDestination());
			else r1.setLocation(edge.getOrigin());
		...
		m2.setParent(this);
		m2.setLocation(edge);
		if(isReversed()) m2.setContainer(edge.getOrigin());
		else m2.setContainer(edge.getDestination());
		...
		if(previousState != null) {
			...
			r2.setPriority(0);
			r2.setParent(this);
			if(isReversed()) r2.setLocation(edge.getOrigin());
			else r2.setLocation(edge.getDestination());
	...
	public double getDuration() {
		if(getSpeed()==0||getDutyCycle()==0) return Double.POSITIVE_INFINITY;
		else return GlobalParameters.getRoundedTime(edge.getDistance()/(getSpeed()*getDutyCycle()*24));
	}
	 ...
	 * 
	 * @return true if the edge's origin and destination are swapped, false
	 * otherwise
	 */
