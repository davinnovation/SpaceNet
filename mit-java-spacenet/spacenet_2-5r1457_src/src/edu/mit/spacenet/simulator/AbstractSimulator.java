...
public abstract class AbstractSimulator implements I_Simulator {
	...
	protected void handleDemands() {
		if(duration > 0) {
			for(I_Element element : getScenario().getNetwork().getRegistrar().values()) {
				...
				d.setDemands(repairDemands(discretizeDemands(element.generateDemands(duration, this), element, element.getLocation()), element));
				if(d.getDemands().size()>0) {
					try {
						...
						if(error instanceof SimSpatialError) {
							...
							/* TODO double-counting? see DemandEvent.execute
							if(isPackingDemandsAdded()) {
								Demand packingDemand = new Demand(new GenericResource(ClassOfSupply.COS5), 0);
								for(Demand demand : ((SimDemand)error).getDemands()) {
		...
		if(event instanceof MissionEvent) {
			...
			d.setDemands(discretizeDemands(mission.generateDemands(this), null, mission.getDestination()));
			schedule(d);
		}
	}
	private DemandSet discretizeDemands(DemandSet demands, I_Element element, Location location) {
		if(getScenario().getItemDiscretization()==ItemDiscretization.BY_ELEMENT
				&& element!=null) {
			for(Demand demand : demands) {
				if(demand.getResource() instanceof Item) {
					...
					demand.setAmount(0);
					while(itemDemandsByElement.get(element).get(item) >= getScenario().getItemAggregation()) {
			...
			for(Demand demand : demands) {
				if(demand.getResource() instanceof Item) {
					...
					demand.setAmount(0);
					while(itemDemandsByLocation.get(location).get(item) >= getScenario().getItemAggregation()) {
						demand.setAmount(demand.getAmount() + 1);
						itemDemandsByLocation.get(location).put(item, 
			...
			for(Demand demand : demands) {
				if(demand.getResource() instanceof Item) {
					...
					itemDemandsByScenario.put(item, 
							itemDemandsByScenario.get(item) + demand.getAmount());
					demand.setAmount(0);
					while(itemDemandsByScenario.get(item) >= getScenario().getItemAggregation()) {
						demand.setAmount(demand.getAmount() + 1);
						itemDemandsByScenario.put(item, 
								itemDemandsByScenario.get(item) - 1);
					}
				}
			}
		}
		...
		if(itemsRepaired) {
			...
			for(Mission mission : getScenario().getMissionList()) {
				if(getScenario().getRepairedItems().get(mission)!=null
						&& getTime() < mission.getDuration() + DateFunctions.getDaysBetween(mission.getStartDate(), getScenario().getStartDate())
						&& getTime() >= lastTime) {
					if(mission.isCrewed()) 
						lastTime = mission.getDuration() + DateFunctions.getDaysBetween(mission.getStartDate(), getScenario().getStartDate());
						...
						if(item.getElement().equals(element)) {
							for(Demand demand : demands) {
								if(item.getDemand().getResource().equals(demand.getResource())
									...
									double amountRepaired = Math.min(demand.getAmount(), item.getDemand().getAmount());
									if(this instanceof FullSimulator) {
										((FullSimulator)this).getCrewTimeHistory().add(
												new MoeCrewTime(DateFunctions.getDaysBetween(mission.getStartDate(), getScenario().getStartDate()), 
														element.getLocation(), 
														MoeCrewTime.CORRECTIVE_MAINTENANCE, 
									...
									getRepairedParts().add(new SimRepair(getTime(),
											(I_Item)demand.getResource(), amountRepaired,
											amountRepaired*item.getUnitMeanRepairTime(),
											amountRepaired*item.getUnitMassToRepair(),
											element.getLocation(), element));
									demand.setAmount(demand.getAmount() - amountRepaired);
									demands.add(new Demand(new GenericResource(ClassOfSupply.COS4), item.getUnitMassToRepair()*amountRepaired));
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.simulator.I_Simulator#getDemandHistory(edu.mit.spacenet.domain.element.I_Element)
	 */
	...
	public List<SimDemand> getDemandHistory(Mission mission) {
		ArrayList<SimDemand> demands = new ArrayList<SimDemand>();
		double startTime = DateFunctions.getDaysBetween(mission.getStartDate(), getScenario().getStartDate());
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.simulator.I_Simulator#getDemandHistory(edu.mit.spacenet.domain.network.Location)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.simulator.I_Simulator#getDemands(edu.mit.spacenet.scenario.Mission, edu.mit.spacenet.domain.ClassOfSupply)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.simulator.I_Simulator#getDemands(edu.mit.spacenet.scenario.Mission, edu.mit.spacenet.domain.element.I_Element)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.simulator.I_Simulator#getDemands(edu.mit.spacenet.scenario.Mission, edu.mit.spacenet.domain.element.I_Element, edu.mit.spacenet.domain.ClassOfSupply)
	 */
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.simulator.I_Simulator#getDemands(edu.mit.spacenet.domain.element.I_Element, edu.mit.spacenet.domain.ClassOfSupply)
	 */
	public DemandSet getDemands(I_Element element, ClassOfSupply cos) {
		...
		for(SimDemand d : getUnsatisfiedDemands()) {
			for(Demand demand : d.getDemands()) {
				if(element.equals(d.getElement())
						&& (demand.getResource().getClassOfSupply().equals(cos)
						|| demand.getResource().getClassOfSupply().isSubclassOf(cos))) {
					demands.add(demand);
				}
			}
	...
	/* (non-Javadoc)
	 * @see edu.mit.spacenet.simulator.I_Simulator#getDemands(edu.mit.spacenet.domain.network.Location, edu.mit.spacenet.domain.ClassOfSupply)
	 */
